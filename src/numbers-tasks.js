...
function getAverage(value1, value2) {
  let res;
  if (value1 + value2 >= Number.MAX_VALUE * 2) {
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  if (
    Math.atan2(x1, y1) + Math.atan2(x2, y2) === 0 ||
    Math.atan2(x1, y1) + Math.atan2(x2, y2) === Math.PI
  ) {
    ress = Math.atan2(x1, y1) + Math.atan2(x2, y2);
  } else {
    ress = Math.abs(Math.atan2(x1, y1) + Math.atan2(x2, y2) - Math.PI);
  }
  return ress;
}
...
function getFibonacciNumber(index) {
  ...
  for (let i = 2; i <= index; i += 1) {
    const res1 = result[i - 1];
    const res2 = result[i - 2];
...
function isNumber(number) {
  if (Number.isNaN(number) || Number.isFinite(number) === false) {
    return false;
  }
  return true;
}
...
function getRandomInteger(min, max) {
  const qwe = Math.ceil(min);
  const qwew = Math.floor(max);
  return Math.floor(Math.random() * (qwew - qwe + 1)) + qwe;
}
...
function getHypotenuse(a, b) {
  if (Math.sqrt(a ** 2 + b ** 2) >= Number.MAX_VALUE) {
